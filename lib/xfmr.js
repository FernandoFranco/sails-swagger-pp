...
var Transformer = {
  ...
  getPaths: function getPaths(sails) {
    ...
    return _lodash2['default'].transform(paths, function(res, v, k) {
      if (v) res[k] = v;
    });
  },
  ...
  getPathItem: function getPathItem(sails, pathGroup, pathkey) {
    ...
    return _lodash2['default'].transform(pathItem, function(res, v, k) {
      if (v) res[k] = v;
    });
  },

  mergeOperationWithCustom: function(sails, methodGroup, operation) {
    ...
    var identity = (route.controller || route.split('.')[0]).replace('Controller', '').toLowerCase();
    var action = route.action || route.split('.')[1];

    var controller = sails.controllers[identity];
    if (!controller || !controller.$swagger || !controller.$swagger[action]) return operation;

    var swaggerAction = controller.$swagger[action][methodGroup.method] || controller.$swagger[action];
    if (swaggerAction.ignore) return undefined;

    return _lodash2['default'].merge(operation, swaggerAction);
  },
